...
static u8 Cx=0,Cy=0,Cw_now=200,Ch=0;
static u8 Cw_last = 200;
#define Cw_min 5 
#define Cw_max 170
		...
		if( USART_GetITStatus(MY_USART,USART_IT_RXNE)!=RESET)  	   //接收中断  
		{
				...
				else if(RxState==2)
				{
					...
					if(RxCounter1>=10||com_data == 0x5B)       //RxBuffer1接受满了,接收数据结束
					{
						...
						Cw_now=RxBuffer1[RxCounter1-3];
						if(Cw_now > Cw_last + 5 || Cw_now + 5 < Cw_last)
						{
							Cw_now = Cw_last;
						}
						Cw_last = Cw_now;
...
uint8_t colorsearch()//判断有无红色
{
	return (Ch);
}

uint8_t x_position()//红色方块的坐标
{
	return (Cx);
}