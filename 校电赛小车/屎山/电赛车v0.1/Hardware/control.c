...
float Distance;
//输出环值
int Out_Displacement,Out_Velocity,Out_turn;//位移环输出值和速度环输出值
int PWM_Out;//最终输出环值
...
void PID_TIM_IRQHandler()
{
	if(TIM_GetITStatus(PID_TIM, TIM_IT_Update) == SET)
	{
		...
		Distance = Send(); //测距离
		turn_dir = 0; Red_x = x_position();//测红色方块x坐标
		...
		Out_turn = Turnment(160, Red_x);
		PWM_Out = Final_PWM_Out(Out_Displacement, Encoder_left, Encoder_right, AX);		
//		PWM_Out = -PWM_Out;
		
		if(colorsearch()) Redflag = 1; else Redflag = 0;//判断是否有红色标志物
		
		//if(Distance<=10 && Redflag == 1)
		if(Distance == Med_Displacement && Redflag == 1)
		{
			static uint8_t i = 0; i++;
			if(i >= 3)
			{
				i = 0;
				TIM_Cmd(PID_TIM,DISABLE); Mode = 3; 
			}
		}		
		
//		if(Red_x <100 || Red_x > 220)//接收标志位
//		{
//			//判断转的方向
//			if(Red_x < 115)
//			{
//				turn_dir = 1;
				//motor1 = PWM_Out + Out_turn; motor2 = PWM_Out - Out_turn;
//			}
//			else if(Red_x > 205)
//			{
//				turn_dir = -1;
...
int Displacement(float Med, float SR04_Num)
{
	...
	//3.积分限幅
	Displacement_Err_S = Displacement_Err_S>7200?7200:(Displacement_Err_S<(-7200)?(-7200):Displacement_Err_S);
	//4.位移环输出计算
...
//	Velocity_Low_Err_last = Velocity_Low_Err;
	...
	//4.代入公式
	PWM_Out = Velocity_Kp * Velocity_Err  + Velocity_Kd * (Acc - 0);
...
{
	static int PWM_Out;
	PWM_Out = Turn_Kp*(x_position-Med);
...
int Final_PWM_Out(float Displacement_PWM_Out, int16_t Encode_A, int16_t Encode_B, float Acc)
{
	...
	//3.计算串级PID最终结果
	PWM_Out = Velocity_Kp*Velocity_reality + Velocity_Kd*Acc - Velocity_Kp*Displacement_PWM_Out;
