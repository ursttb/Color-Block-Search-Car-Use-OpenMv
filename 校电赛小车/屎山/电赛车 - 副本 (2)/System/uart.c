...
#define MY_USART USART1//串口选择
#define RCC_MY_USART RCC_APB2Periph_USART1//串口使能,记得在代码修改ABPx
...
#define USART_IRQN USART1_IRQn//中断设置
...
void USARTx_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(RCC_MY_USART, ENABLE);	
	RCC_APB2PeriphClockCmd(RCC_MY_USART_GPIO, ENABLE);   //打开PA端口时钟

    //USART1_TX   PA9
    GPIO_InitStructure.GPIO_Pin = USART_Pin_TX;          		 //PA9
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;  		 //设定IO口的输出速度为50MHz
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;	   		 //复用推挽输出
    GPIO_Init(MY_USART_GPIO, &GPIO_InitStructure);             	 	 //初始化PA9
    //USART1_RX	  PA10
    GPIO_InitStructure.GPIO_Pin = USART_Pin_RX;             //PA10
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;  //浮空输入
    GPIO_Init(MY_USART_GPIO, &GPIO_InitStructure);                 //初始化PA10 
		...
		if( USART_GetITStatus(USART1,USART_IT_RXNE)!=RESET)  	   //接收中断  
		{
				...
				else if(RxState==3)		//检测是否接受到结束标志
				{
						if(RxBuffer1[RxCounter1-1] == 0x5B)
						{
									...
									{
									OLED_Clear();
									OLED_ShowNum(1, 0,Cx,3);
									OLED_ShowNum(2,17,Cy,3);
									OLED_ShowNum(3,33,Cw,3);
									OLED_ShowNum(4,49,Ch,3);
									}
									RxFlag1 = 0;
									RxCounter1 = 0;
									RxState = 0;
